...
authRouter.post("/lawyerSignup", async (req, res) => {
  try {
    ...
    if (
      ...
      loggers.error("Lawyer signup failed: Missing required fields", {
        body: req.body,
      });
      return res.status(400).json({
        status: "failed",
    ...
    if (MOBILENUMBER.length !== 10 || !/^\d+$/.test(MOBILENUMBER)) {
      loggers.error("User signup failed: Invalid phone number format", {
    ...
    const checkMobileNumber = await lawyerAuth.findOne({ MOBILENUMBER });

    if (checkMobileNumber) {
      loggers.info("Lawyer signup failed: Account already registered", {
        MOBILENUMBER: MOBILENUMBER,
      });
      return res.status(400).json({
        status: "failed",
    ...
    const pastUser = await UserAuth.findOne({ MOBILENUMBER: MOBILENUMBER });
    if (pastUser) {
    ...
    loggers.info("Lawyer signed up successfully", {
      MOBILENUMBER: MOBILENUMBER,
    });
    ...
    console.log("New Lawyer data added successfully", {
      MOBILENUMBER: MOBILENUMBER,
    });
...
authRouter.post("/lawyerLogin", async (req, res) => {
  ...
  try {
    if (!EMAIL || !PASSWORD) {
      loggers.error("Lawyer login failed: Missing required fields", {
        body: req.body,
      });
      return res.status(400).json({
        status: "failed",
